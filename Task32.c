...
void printRound(Queue**q, FILE*fout, Stiva**winners, Stiva**losers, int *counter){
    Team aux1, aux2;
    while(!isEmptyQ(*q)){
        aux1=deQueue(*q);
        aux2=deQueue(*q);
...
Node* Task3(FILE* fout, Node* head){
    ...
    while(counter != 1){
        ...
        fprintf(fout, "\n--- ROUND NO:%d\n", round);
        printRound(&q, fout, &winners, &losers, &counter);
        ...
        /*if(counter == 8) {
            ...
            Queue *qAux=q;
            while(qAux->front){
                printf("%s\n", (qAux->front)->echipa.numeEchipa); 
                qAux->front=(qAux->front)->next;
            }
            //creareLista(&last8, q);
        }*/
    }
